...
# bot = telegram.Bot(config.get('bot', 'token'))
...
def parse(content):
    ...
    if status.lower() != 'actual':
        ...
        if type(parse) == list:
            for x in parse:
                # msgfromat(x)
                threading.Thread(target=msgfromat, args=[x]).start()
        else:
            # msgfromat(parse)
            threading.Thread(target=msgfromat, args=[parse]).start()
    ...
    *備註*
    ...
    for pics in parse_['resource']:
        if pics['resourceDesc'] == '地震報告圖':
            ...
            # pic_ = requests.get(pic, stream=True)
    ...
    def getpic(url):
        ...
        if pic_.status_code == 200:
            pic_.decode_content = True
            return io.BytesIO(pic_.content)
    for chatId in chatIdList:
        while True:
            try:
                bot.sendPhoto(
                    int(chatId), getpic(pic)
                )
                bot.sendMessage(
